...
from ..utils import auto_convert_objectid, auto_convert_objectid_list, ObjectId
...
class QueryFilter:
    ...
    def _validate_current_field(self) -> None:
        ...
        if not self._current_field:
            raise QueryFilterError(
                "No field selected. Call field() before applying a condition."
            )
    ...
    def between(
        ...
        if inclusive:
            self._filters[self._current_field] = {
                Operator.GTE: min_value,
                Operator.LTE: max_value
            }
        else:
            self._filters[self._current_field] = {
                Operator.GT: min_value,
                Operator.LT: max_value
            }
        return self
    ...
    def text_search(self, search_text: str, language: Optional[str] = None, 
                    case_sensitive: bool = False, diacritic_sensitive: bool = False) -> 'QueryFilter':
        """
